...
Close the Unreal editor before building. Do not use Live Coding or `LiveCoding.Compile` for Codex-driven builds. If UnrealBuildTool reports that Live Coding is active, close the editor, terminate any leftover `LiveCodingConsole.exe`, verify no `UnrealEditor.exe` process remains, and rerun the normal build command below.
