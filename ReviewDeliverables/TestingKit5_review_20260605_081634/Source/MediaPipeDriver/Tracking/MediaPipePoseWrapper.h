...
public:
	...
	void Unload();
	bool Init(const FString& PoseModelPath, const FString& HandModelPath = FString(), const FString& HolisticModelPath = FString(), const FMediaPipePoseNativeOptions& Options = FMediaPipePoseNativeOptions());
	bool InitMock();
	...
	bool GetFacePose(FMediaPipeRawFacePose& OutFace);
	void Shutdown();

	bool IsLoaded() const;
	bool IsReady() const;
	bool AreHandsEnabled() const { return bHandsEnabled; }
	bool IsFaceEnabled() const { return bFaceEnabled; }
...
private:
	...
	typedef bool (*MP_Init4Fn)(const char* PoseModelPath, const char* HandModelPath, const char* HolisticModelPath, const FMediaPipeNativeInitOptions* Options);
	...
	typedef bool (*MP_GetFacePoseFn)(FMediaPipeRawFacePose* OutFace);
