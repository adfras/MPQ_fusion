...
- **GATE-AUTO** — headless automation, exit code 0, expected count met:
  ...
  -ExecCmds="Automation RunTests TestingKit5.MediaPipe+TestingKit3.MediaPipe.BodyFusion+TestingKit3.MediaPipe.BodySolverMath; Quit"
  -TestExit="Automation Test Queue Empty"` = **54/54**.
  After Phase 6 (prefix migration) the filter becomes `TestingKit5.MediaPipe` and the expected
  count becomes **110/110** (92 renamed + 18 existing; measured inventory, section 3.6).
