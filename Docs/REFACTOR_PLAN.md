...
Status: authored and EXECUTED 2026-06-11 through Phase 13 (see the execution log, section 8).
Remaining follow-ups, in order: Phase 11b (live-profile tables) + the Phase 12 deletion
(decision recorded), 11c (capture scopes), 11d (guard demotion), and the optional inner
dataset/MPQ split of `Diagnostics/MediaPipeCaptureRecorders.cpp` (Phase 9 note).
Sections 3–4 below describe the PRE-execution tree (commit `0e233f0`); their file/line
anchors are historical evidence, not current positions — the execution log records what
actually changed and any deviations.
...
| ----- | ---- | ---------- | ----- | ----- |
| 13 | 2026-06-11 | (commit below) | PY 20/20 + selftests (docs-only phase) | AGENTS.md: replay map + canonical dataset + PIE gate tooling added to the primary-workflow list; bridge port-ownership note (8765 may belong to another project; use `CODEX_AGENT_PORT=8766`). `Docs/README.md`: `CVAR_REFERENCE.md` listed with regeneration command. Plan header updated: executed through Phase 13, sections 3–4 marked as pre-execution historical anchors, follow-up order recorded (11b+12-deletion, 11c, 11d, optional recorder inner split). |
| 12 | 2026-06-11 | (decision record; code unchanged) | Reachability verified in source (no gates needed for a decision record) | **DECISION: DELETE** the leg target-IK branch and foot-plant LOCK. Evidence: `MediaPipePoseDrivenAnimInstance_LegSolve.cpp:69-72` — `DriveLegCS` returns before any IK code when `bDriveLegs` is false; both live profiles set `mp.MediaPipeDriveLegs=0` (`MediaPipeDriverRuntime.cpp:582`), and the replay policy sets `DriveLegs=1` but forces `UseLegIK=0`/`FootPlant=0`. So `bDoLegIK` (`:502`) is reachable by NO profile, map, or test — only a hand-typed console combo. Measured-worse evidence + zero Content references already in section 3.3. CARVE CAUTION for the executing change: the foot-plant STATE tracking (`bFootPlantLocked`, `bCurrentSourceFootGrounded`, `bCurrentSourceFootNearFloor`, ~:260-330) feeds the LIVE FK-root-grounding eligibility (2026-06-10 fix) and MUST be kept; delete only the `bDoLegIK` solve branch (~:502-660), the lock APPLICATION, and the `LegIK*` tunables, then drop `UseLegIK=1` from the stable-profile table when 11b ports it. Execute together with 11b (same files, same gate cycle: full gates + PIE). |
| 11a | 2026-06-11 | `f548171` | BUILD ok; AUTO 112/112; PY green; PIE PASS both avatars; **effective-CVar dump diff EMPTY** (417 rows pre vs post; only artifact = the sampler's own seek offset). Stack log: `apply=ReplayEvaluation prio=3 settings=23 written=23 coveredByHigher=0` | New `Runtime/MediaPipeCVarPolicy.h/.cpp`: priority stack (Baseline<LiveProfile<CaptureScope<ReplayEvaluation), one applier with transition logging, higher-layer coverage check. `ApplyReplayPoseCVars_GameThread` now builds the ReplayEvaluation layer — value-for-value the invariant-2 contract (23 settings, comments preserved). Reassert guard unchanged (idempotent re-apply through the stack). **11b–11d NOT executed in this session** — deliberate stop per the plan's quality rule: porting the ~490-line `ApplyAutoQuestProfile` + 2 live profiles into LiveProfile tables (11b) is a 300+-entry transcription with full per-sub-phase gates; the stack, the dump-diff procedure (`cvar_dump_pre/post_*.txt` + DumpCVars-in-PIE), and the multi-writer worklist (CVAR_REFERENCE) are in place for it. 11c = capture scopes (now in `MediaPipeCaptureRecorders.cpp`), 11d = guard demotion after 11b+11c hold. |
| 10 | 2026-06-11 | `a5b2a4b` | BUILD green (one fix: an off-by-one duplicated namespace brace in the generated shared header); AUTO 112 discovered, 111 completed + the queued-quit straggler (`RuntimeProfile.MediaPipeOnlyEmbodiedWebcamShoulderShrugDefaults`, untouched file) verified passing in isolation; PY + selftests; **PIE PASS both avatars, 1,980 samples, knee extremes exact** | The ~14k-line single TU is gone. New layout: main `MediaPipePoseDrivenAnimInstance.cpp` 2,576 lines; solver TUs `_QuestArmSolve` 3,615 / `_QuestHandRotation` 1,848 / `_BodyPoseSolve` 1,490 / `_QuestSpaceMapping` 1,053 / `_QuestFingerDrive` 999 / `_ReferenceCache` 902 / `_LegSolve` 779 / `_ArmTwist` 519 / `_TorsoBasis` 252; shared internal-linkage helper header `MediaPipePoseDrivenAnimInstanceShared.h` 888 lines (anonymous-namespace helpers + statics + the three using-directives, per-TU copies = pre-split semantics). `Inline/` deleted; Build.cs include path removed; `_BodyState.inl` (6 lines) folded into main; embedded automation test kept in main TU only (single registration). | 
| 9 | 2026-06-11 | `d5d2987` | BUILD clean on FIRST compile; AUTO 112/112; PY + selftests; **PIE PASS both avatars, 1,982 samples (30 Hz), knee minima exact** | ADAPTED from the planned 5-file split: all ~4,250 tooling lines moved verbatim into ONE focused TU `Diagnostics/MediaPipeCaptureRecorders.cpp` (4,784 lines incl. includes/shims) behind a 5-symbol header (`RecordMannyBoneTimeseriesSample`, 2 Stops, `ResolveTrackingSourceActor`, `LiveMannyTag` + 2 enums). Rationale: the dataset and MPQ/Manny sections share globals and helpers (e.g. `ArmMPQShadowAutoStartRequest` writes `GMannyBoneTimeseriesRecorder` state), so the single-seam move was the verbatim-safe cut; the inner dataset-vs-MPQ cluster split is a documented follow-up. Actor file: **5,405 → 685 lines** (actor + placement helpers only). |
| 8 | 2026-06-11 | `79956cf` (8a+8b), `92bcc8a` (8c) | BUILD ok; AUTO 112/112; PY green; **PIE PASS both avatars at full 30 Hz** (1,981 samples; knee L/R, maxes, ball medians ±0.01, 0 penetration, drift 0.0 — confirms Phase 4's 8 Hz aliasing was environmental) | 8a SCOPE NARROWED vs plan: only the hard-false evaluation hooks, the OR-term call site, and write-only `SmoothedStage1ChestOffset` state removed. The Stage1 capture-ARMING plumbing (3 `mp.BodyFusion.Stage1*` CVars, MPQ trial `stage1=` args, snapshot fields) KEPT — live API surface asserted by `Runtime.CVars` + `PrepareMPQShadowLatencyTrial` tests, referenced by the active Stage2A protocol doc, consumed by the C++-invoked MPQ analyzer; its removal belongs with an MPQ-shadow-capture deprecation decision. 8b removed with tombstone comment. 8c: component + CVar + `ControlRig` Build.cs dependency removed; −145 lines net across the phase. |
| 7 | 2026-06-11 | `28a0415` | BUILD ok; AUTO 112/112; PY 20/20 + plot selftest + generator selftest | `Tools/GenerateCVarReference.py` (+`--selftest`, now part of GATE-PY) emits `Docs/CVAR_REFERENCE.md`: **415** mp.* definitions (415 incl. editor module), **143** file-local, **175 multi-writer**. Relocation judgment: NO definitions moved — every invariant-2 replay-policy CVar is already centrally defined; the cross-file-written file-locals are the `Record*`/`RegionQuality*` capture-tooling families whose owning sections move wholesale in Phase 9, so relocating now would be double churn. The multi-writer table is the Phase 11 worklist. |
