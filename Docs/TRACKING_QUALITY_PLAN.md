...
| worn-1 | 0cb1189 | 194/194 | First candidate worn session (2026-07-12) surfaced two shrug-corrector defects, both fixed and worn-ACCEPTED same day. (a) Shoulder drift: the squat/knee-raise segment compressed measured shoulder-above-hip height to ~40cm; the UNGATED 2.5s down-adapt learned it as rest (mp.ClavicleShrugFusion: L restRef 47->40 while heightCm returned to 46-53) and the 2026-07-09 quiet up-gate froze recovery at 600s - Kellan held a 6-12cm left shrug at rest (worn screenshot + rows). Fix: mp.ShrugRestRefDownGate (default 0, candidate 1) - drops deeper than mp.ShrugRestRefDownBandCm (2.5) below rest learn at 600s, symmetric with the up direction; rows gain elevCm=/dnGated=. (b) Arms-down shrugs invisible: a real trap shrug moves the webcam shoulder LANDMARK only 1.5-3cm (rows 00:12:41-44: peak appliedCm 3.5 -> 0.5 while the shrug was held) vs 5-9cm on arm raises; the fixed 1.5cm knee ate it. Fix: mp.ShrugDeadbandCm (default 1.5 = legacy) + mp.ShrugLiftGain (default 1.0, POST-knee so rest noise stays zeroed; candidate 1.0/1.5x). Proofs: +5 behavioral tests (squat-poisoning repro, gated squat, in-band settle still fast, 2cm-shrug recovery, gain linearity); ClavicleShrugGolden untouched green (its C3 -3cm drop locks the disarmed down path). Worn verdict rows (00:59-01:01): L restRef held 44.4-45.4cm across the session, dnGated fired once, user verdict good. |
