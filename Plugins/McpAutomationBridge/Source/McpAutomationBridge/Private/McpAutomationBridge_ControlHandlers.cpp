  ...
  if (Payload->TryGetObjectField(TEXT("properties"), PropertiesPtr) &&
    ...
    for (const auto &Pair : (*PropertiesPtr)->Values) {
      ...
      if (ApplyJsonValueToProperty(NewComponent, Property, Pair.Value,
                                   ApplyError))
        AppliedProperties.Add(FString(Pair.Key.ToView()));
      else
...
bool UMcpAutomationBridgeSubsystem::HandleControlActorSetComponentProperties(
  ...
  FString MobilityKey;
  for (const auto &Pair : (*PropertiesPtr)->Values) {
    if (FString(Pair.Key.ToView()).Equals(TEXT("Mobility"), ESearchCase::IgnoreCase)) {
      MobilityVal = &Pair.Value;
      MobilityKey = FString(Pair.Key.ToView());
      break;
    }
  }
  ...
  for (const auto &Pair : (*PropertiesPtr)->Values) {
    ...
    if (FString(Pair.Key.ToView()).Equals(TEXT("SimulatePhysics"), ESearchCase::IgnoreCase) ||
        FString(Pair.Key.ToView()).Equals(TEXT("bSimulatePhysics"), ESearchCase::IgnoreCase)) {
...
bool UMcpAutomationBridgeSubsystem::HandleControlActorSetBlueprintVariables(
  ...
  for (const auto &Pair : (*VariablesPtr)->Values) {
    ...
    if (ApplyJsonValueToProperty(Found, Property, Pair.Value, ApplyError))
      Applied.Add(FString(Pair.Key.ToView()));
    else
...
bool UMcpAutomationBridgeSubsystem::HandleControlEditorPlay(
  ...
  bool bVRPreview = false;
  Payload->TryGetBoolField(TEXT("vrPreview"), bVRPreview);
  if (!bVRPreview) {
    FString PreviewType;
    if (Payload->TryGetStringField(TEXT("previewType"), PreviewType)) {
      bVRPreview = PreviewType.Equals(TEXT("vr"), ESearchCase::IgnoreCase) ||
                   PreviewType.Equals(TEXT("vrPreview"), ESearchCase::IgnoreCase) ||
                   PreviewType.Equals(TEXT("inVR"), ESearchCase::IgnoreCase);
    }
  }
  if (bVRPreview) {
    PlayParams.SessionPreviewTypeOverride = EPlaySessionPreviewType::VRPreview;
  }
  ...
  TSharedPtr<FJsonObject> Resp = McpHandlerUtils::CreateResultObject();
  Resp->SetBoolField(TEXT("success"), true);
  Resp->SetBoolField(TEXT("vrPreview"), bVRPreview);
  SendAutomationResponse(Socket, RequestId, true,
                         bVRPreview ? TEXT("VR Preview started")
    ...
    for (const auto& Pair : (*PrefsPtr)->Values) {
      ...
      if (CVar) {
        ...
        if (Pair.Value->TryGetString(Value)) {
          CVar->Set(*Value);
          AppliedSettings.Add(FString(Pair.Key.ToView()));
        } else {
          double NumVal;
          if (Pair.Value->TryGetNumber(NumVal)) {
            ...
            if (Pair.Value->TryGetBool(BoolVal)) {
              ...
              FailedSettings.Add(FString(Pair.Key.ToView()));
            }
          }
        }
      } else {
