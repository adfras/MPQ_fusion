...
void FAnimNode_MediaPipePoseDriven::DriveArmCS(FCSPose<FCompactPose>& CSPose, bool bIsLeft, float DeltaSeconds)
{
	...
	else if (bMetaHumanFullArmChainDirectFresh)
	{
		...
		if (ArmDirectionWeight > 0.0f && RuntimeStateKey != 0)
		{
			...
			QuestWristSideState.ArmDirMotionFadeAlpha = FMath::Lerp(
				QuestWristSideState.ArmDirMotionFadeAlpha,
				MotionFadeTarget,
				HalfLifeToAlpha(0.2f, DirStepSeconds));
			const float DirApplyAlpha = DirAlpha * QuestWristSideState.ArmDirMotionFadeAlpha;
				...
				!ChainElbowOffset.IsNearlyZero() && !ChainWristOffset.IsNearlyZero())
			{
				...
				if (!CamElbowDir.IsNearlyZero() && !CamWristDir.IsNearlyZero())
				{
					...
					{
						const int32 OtherShoulderLmIdx = bIsLeft
						...
						if (IsMeasured(OtherShoulderLmIdx) &&
							TryGetLmWorld(OtherShoulderLmIdx, OtherShoulderWorld))
						{
							const FVector ShoulderLine = bIsLeft
								? (OtherShoulderWorld - MediaPipeSourceShoulderWorld)
								: (MediaPipeSourceShoulderWorld - OtherShoulderWorld);
					...
					UpdateCorrection(ElbowDelta, QuestWristSideState.ArmDirCorrectionElbow,
				...
				!ChainElbowOffset.IsNearlyZero() && !ChainWristOffset.IsNearlyZero())
			{
				...
				const FQuat ScaledElbow = FQuat::Slerp(
					FQuat::Identity, QuestWristSideState.ArmDirCorrectionElbow, DirApplyAlpha);
				const FQuat ScaledWrist = FQuat::Slerp(
					FQuat::Identity, QuestWristSideState.ArmDirCorrectionWrist, DirApplyAlpha);
